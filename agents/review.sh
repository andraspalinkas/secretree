#!/bin/sh
# secretree review agent: run by `secretree runner --name ai-review --cmd agents/review.sh`
...
set -eu
[ -n "${SECRETREE_PR:-}" ] || { echo "not a pull request commit; nothing to review"; exit 0; }
...
diff=$(git diff "${SECRETREE_BASE}...${SECRETREE_COMMIT}")
[ -n "$diff" ] || { echo "empty diff"; exit 0; }

# the diff leaves the key boundary: say so where the team can see it
secretree -C "$SECRETREE_REPO_DIR" ledger add --kind export --subject "diff of PR #${SECRETREE_PR} @${SECRETREE_COMMIT} → ${PROVIDER}" --note "ai review" >/dev/null
...
echo "$out" | while IFS= read -r line; do
  case "$line" in
    \{*\"verdict\"*)
      ...
      secretree -C "$SECRETREE_REPO_DIR" pr review "$SECRETREE_PR" --verdict "$v" -m "$s"
      echo "verdict: $v — $s";;
    \{*\"path\"*)
      p=$(printf '%s' "$line" | jq -r .path); l=$(printf '%s' "$line" | jq -r .line); c=$(printf '%s' "$line" | jq -r .comment)
      secretree -C "$SECRETREE_REPO_DIR" pr comment "$SECRETREE_PR" --path "$p" --line "$l" -m "$c" >/dev/null
      echo "comment: $p:$l";;
  esac
done
echo "review posted for #${SECRETREE_PR} @${SECRETREE_COMMIT}"
