...
secretree they run as jobs on your runner and report back as signed checks
or pull requests. Nothing changes for the tool; what changes is who gets to
read the tree.

The runner executes `.secretree/ci` (or `make ci`, or `--cmd`). Add steps
there. Each step can create extra checks by running `secretree`
itself (the runner's clone holds a key), or simply fail the job.

## Dependency updates (Renovate)

```sh
# in .secretree/ci, or as a separate runner with --name deps --cmd:
...
branch, commit the bump, `git push origin`, then `secretree pr open`. A
`secretree integrate renovate` recipe that wraps this is planned.
...
```sh
git diff "$SECRETREE_COMMIT~1..$SECRETREE_COMMIT" | my-review-tool > review.txt
secretree pr comment "$PR" -m "$(cat review.txt)"
```
...
effect of hosting. Record the provider in `.secretree/policy.json` or the
ledger so the team knows.

## Deploy previews

Run `secretree deploy-agent --branch <pr-branch> --to /srv/previews/<n>` on
...
ntfy, Slack or e-mail; the recipient runs `secretree pr list` or opens the
