...
REPO="$(cd "$(dirname "$0")/.." 2>/dev/null && pwd || echo .)"
if [ "${1:-}" = "--clean" ]; then
...
if [ -n "${SECRETREE_BIN:-}" ]; then
  # `secretree demo`: use the running binary, no Go toolchain needed
  BINDIR="$TOUR/bin"; mkdir -p "$BINDIR"
  ln -sf "$SECRETREE_BIN" "$BINDIR/secretree"; ln -sf "$SECRETREE_BIN" "$BINDIR/git-remote-secretree"
...
Stop the UI:  pkill -f "secretree ui"      Remove everything:  secretree demo --clean --dir "$TOUR"   (or scripts/tour.sh --clean)
MSG
